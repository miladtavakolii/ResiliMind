...
class TurnPrediction(BaseModel):
    """
    Stores raw prediction generated by ResiliMind for a single conversation turn.

    This model keeps the complete intermediate workflow output required
    ...
    active_nodes: list[Any] = Field(
        default_factory=list
    )

    active_signals: list[Any] = Field(
        default_factory=list
    )
...
class CasePrediction(BaseModel):
    """
    Stores the complete raw output of ResiliMind for one evaluation case.

    A case can contain multiple conversation turns and the final generated
    advisor response.
    """
    ...
    turns: list[TurnPrediction] = Field(
        default_factory=list
    )
